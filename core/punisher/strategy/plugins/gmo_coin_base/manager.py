"""
GmoCoinBaseManager — GMO Coin 레버리지 공통 실행 매니저.

MarginTrendManager(CdfTrendFollowingManager) 상속.
GMO Coin 어댑터와 호환되지 않는 메서드 오버라이드:
  ...
  - _detect_existing_position: DB SL 복원 + 거래소 ロスカット 재동기화
  - _pre_entry_checks: GMO Coin 전용 체크 (keep_rate/FX 휴장 없음)
...
상속 체인:
    BaseTrendManager → MarginTrendManager → GmoCoinBaseManager
...
class GmoCoinBaseManager(CfdTrendFollowingManager):
    """GMO Coin 레버리지 공통 실행 매니저. 롱/숏 양방향 지원."""

    # 서브클래스에서 반드시 정의
    _task_prefix = "gmoc_base"
    _log_prefix = "[GmoBaseMgr]"
    ...
    async def _close_position_impl(self, product_code: str, reason: str) -> None:
        ...
        try:
            ...
            if "ERR-422" in str(e):
                ...
                prev_pos = self._position.get(product_code)
                prev_db_id = prev_pos.db_record_id if prev_pos else None
                side = prev_pos.extra.get("side", "buy") if prev_pos else "buy"
                close_size = prev_pos.entry_amount if prev_pos else 0
                self._position[product_code] = None
                if prev_db_id:
                    try:
