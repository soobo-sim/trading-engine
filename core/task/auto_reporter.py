    ...
    async def _generate_report(
        ...
        if report and report.get("success"):
            try:
                ...
                ok_count = sum(
                    1 for c in safety_report.checks if c.status in ("ok", "n/a")
                )
                ...
                if safety_report.status == "all_ok":
                    ...
                    critical_names = [
                        c.name for c in safety_report.checks if c.status == "critical"
                    ]
                    summary = f"🛡️ 안전장치: 🔴 {', '.join(critical_names)} ({ok_count}/{total}) — 즉시 확인 필요"
                else:
                    warn_names = [
                        c.name for c in safety_report.checks if c.status == "warning"
                    ]
