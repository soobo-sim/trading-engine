...
# ── _apply_bias (BoxMeanReversionManager 통합) ───────────────

class TestApplyBias:
    """manager._apply_bias 동작을 intermarket 로직 관점에서 검증."""

    @pytest.mark.asyncio
    async def test_apply_bias_skip_returns_none(self):
        """bias=bearish + direction=long + action=skip → None 반환."""
        from core.analysis.intermarket import IntermarketClient
        from core.strategy.plugins.box_mean_reversion.manager import BoxMeanReversionManager

        mock_client = MagicMock(spec=IntermarketClient)
        mock_client.get_direction_bias = AsyncMock(return_value=("bearish", 0.8, ["test"]))

        manager = MagicMock(spec=BoxMeanReversionManager)
        manager._intermarket_client = mock_client
        # _apply_bias를 실제 메서드로 교체
        manager._apply_bias = BoxMeanReversionManager._apply_bias.__get__(manager)

        params = {
            "intermarket_bias_enabled": True,
            "bias_action": "skip",
            "position_size_pct": 50.0,
        }
        result = await manager._apply_bias("usd_jpy", params, "long")
        assert result is None

    @pytest.mark.asyncio
    async def test_apply_bias_reduce_size(self):
        """bias=bearish + direction=long + action=reduce_size → 사이즈 축소."""
        from core.analysis.intermarket import IntermarketClient
        from core.strategy.plugins.box_mean_reversion.manager import BoxMeanReversionManager

        mock_client = MagicMock(spec=IntermarketClient)
        mock_client.get_direction_bias = AsyncMock(return_value=("bearish", 0.6, ["rate_diff"]))
        ...
        params = {
            "intermarket_bias_enabled": True,
            "bias_action": "reduce_size",
            "bias_size_factor": 0.5,
            "position_size_pct": 50.0,
        }
        result = await manager._apply_bias("usd_jpy", params, "long")
        ...
        # 원본 params 불변 확인
        assert params["position_size_pct"] == 50.0

    @pytest.mark.asyncio
    async def test_apply_bias_aligned_direction_passes_through(self):
        """bias=bullish + direction=long → opposed 아님 → params 그대로."""
        from core.analysis.intermarket import IntermarketClient
        from core.strategy.plugins.box_mean_reversion.manager import BoxMeanReversionManager

        mock_client = MagicMock(spec=IntermarketClient)
        mock_client.get_direction_bias = AsyncMock(return_value=("bullish", 0.7, ["rate"]))
        ...
        result = await manager._apply_bias("usd_jpy", params, "long")
        assert result is params  # 동일 객체

    @pytest.mark.asyncio
    async def test_apply_bias_disabled_passes_through(self):
        """intermarket_bias_enabled=False → _apply_bias 즉시 통과."""
        from core.analysis.intermarket import IntermarketClient
        from core.strategy.plugins.box_mean_reversion.manager import BoxMeanReversionManager

        mock_client = MagicMock(spec=IntermarketClient)
        mock_client.get_direction_bias = AsyncMock(return_value=("bearish", 0.9, ["test"]))

        manager = MagicMock(spec=BoxMeanReversionManager)
        manager._intermarket_client = mock_client
        manager._apply_bias = BoxMeanReversionManager._apply_bias.__get__(manager)

        params = {"position_size_pct": 50.0}  # intermarket_bias_enabled 없음
        result = await manager._apply_bias("usd_jpy", params, "long")
        assert result is params
        # API가 호출되지 않아야 함
        mock_client.get_direction_bias.assert_not_called()

    @pytest.mark.asyncio
    async def test_apply_bias_no_client_passes_through(self):
        """intermarket_client=None → _apply_bias 즉시 통과."""
        from core.strategy.plugins.box_mean_reversion.manager import BoxMeanReversionManager

        manager = MagicMock(spec=BoxMeanReversionManager)
        manager._intermarket_client = None
        manager._apply_bias = BoxMeanReversionManager._apply_bias.__get__(manager)

        params = {"intermarket_bias_enabled": True, "position_size_pct": 50.0}
        result = await manager._apply_bias("usd_jpy", params, "long")
        assert result is params
