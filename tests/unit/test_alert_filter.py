...
def test_tc1_has_position_always_sends():
    ...
    assert _should_send_no_position(pair, alert, "long_setup") is True


# ─── TC-2: 포지션 없음 + regime 동일 + RSI 극단 → 묵음 ──────
def test_tc2_no_pos_same_regime_rsi_extreme_silent():
    pair = "BTC_JPY_tc2"
    _write_cache(pair, "long_setup", time.time())
    alert = {"triggers": ["rsi_extreme_low"], "level": "critical"}
    assert _should_send_no_position(pair, alert, "long_setup") is False
...
def test_tc7_no_pos_24h_elapsed_sends():
    pair = "BTC_JPY_tc7"
    _write_cache(pair, "long_setup", time.time() - _NO_POS_COOLDOWN_SEC - 1)
    alert = {"triggers": ["rsi_extreme_low"], "level": "critical"}
    assert _should_send_no_position(pair, alert, "long_setup") is True
...
def test_tc8_regime_none_no_false_transition():
    ...
    _write_cache(pair, "long_setup", time.time())
    alert = {"triggers": ["rsi_extreme_low"], "level": "critical"}
    # regime=None → unknown → prev "long_setup"와 비교 안 함 → 쿨다운 → 묵음
...
def test_tc10_restart_persistence():
    ...
    _write_cache(pair, "long_setup", time.time() - 100)  # 100초 전 발송
...
def test_normalize_valid():
    assert _normalize_regime("long_setup") == "long_setup"
