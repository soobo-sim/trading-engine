...
테스트 케이스 설계 (재설계 분석서 기반):
  - advisory 미만료 + long_setup signal → entry_long 진입
  - advisory entry_long + hold signal → hold (타이밍 미충족)
  - advisory hold + long_setup signal → hold (레이첼 보류 존중)
  ...
  - entry_short advisory + short_setup signal → entry_short 진입
...
@pytest.mark.asyncio
async def test_entry_long_advisory_with_long_setup_signal():
    """
    Given: advisory=entry_long(미만료), signal=long_setup, 포지션 없음
    When:  decide()
    ...
    with patch("core.judge.decision.rachel_advisory.datetime") as mock_dt:
        ...
        result = await dec.decide(_snapshot(signal="long_setup"))
...
@pytest.mark.asyncio
async def test_entry_short_advisory_with_short_setup_signal():
    """
    Given: advisory=entry_short(미만료), signal=short_setup
    When:  decide()
    ...
    with patch("core.judge.decision.rachel_advisory.datetime") as mock_dt:
        mock_dt.now.return_value = _NOW
        result = await dec.decide(_snapshot(signal="short_setup"))
...
async def test_entry_short_advisory_with_wrong_signal_returns_hold():
    """
    Given: advisory=entry_short, signal=long_setup (방향 불일치)
    When:  decide()
    Then:  action=hold
    """
    adv = _advisory(action="entry_short")
...
async def test_hold_advisory_overrides_entry_signal():
    """
    Given: advisory=hold, signal=long_setup
    When:  decide()
...
async def test_exit_warning_signal_always_exits():
    ...
    with patch("core.judge.decision.rachel_advisory.datetime") as mock_dt:
        ...
        result = await dec.decide(
            _snapshot(signal="hold", exit_action="long_caution", position=_pos())
        )
...
async def test_entry_long_advisory_with_existing_position_auto_converts_to_add_position():
    ...
    with patch("core.judge.decision.rachel_advisory.datetime") as mock_dt:
        mock_dt.now.return_value = _NOW
        result = await dec.decide(_snapshot(signal="long_setup", position=_pos()))
...
async def test_advisory_without_size_pct_uses_none():
    """
    Given: advisory.size_pct=None (재량에 맡김), entry_long + long_setup
    When:  decide()
...
async def test_p01_add_position_profitable_under_limit():
    ...
    with patch("core.judge.decision.rachel_advisory.datetime") as mock_dt:
        ...
        result = await dec.decide(_snapshot(signal="long_setup", position=pos))
...
async def test_p03_add_position_blocks_when_loss_position():
    ...
    with patch("core.judge.decision.rachel_advisory.datetime") as mock_dt:
        ...
        snap = _snapshot(signal="long_setup", position=pos)
...
async def test_p04_add_position_blocks_when_exit_signal_active():
    ...
    with patch("core.judge.decision.rachel_advisory.datetime") as mock_dt:
        ...
        pos = _pos_with_pyramid(pyramid_count=0, unrealized_pnl=200_000.0)
        result = await dec.decide(
            _snapshot(signal="long_setup", exit_action="tighten_stop", position=pos)
        )
...
async def test_p06_add_position_without_open_position_returns_hold():
    ...
    with patch("core.judge.decision.rachel_advisory.datetime") as mock_dt:
        mock_dt.now.return_value = _NOW
        result = await dec.decide(_snapshot(signal="long_setup", position=None))
...
async def test_p08_add_position_full_exit_signal_blocks():
    ...
    with patch("core.judge.decision.rachel_advisory.datetime") as mock_dt:
        ...
        result = await dec.decide(
            _snapshot(signal="long_setup", exit_action="full_exit", position=pos)
        )
...
@pytest.mark.asyncio
async def test_ho01_hold_override_long_setup_returns_entry_long():
    """
    HO-01: advisory=hold, policy=signal_long_setup, signal=long_setup, 포지션 없음
    → action=entry_long, confidence 30% 할인(0.65×0.7=0.455)
    """
    adv = _advisory(action="hold", confidence=0.65, hold_override_policy="signal_long_setup")
    dec, _ = _make_decision(advisory=adv)
...
@pytest.mark.asyncio
async def test_ho02_hold_override_short_setup_returns_entry_short():
    """
    HO-02: advisory=hold, policy=signal_long_setup, signal=short_setup, 포지션 없음
...
async def test_ho03_hold_override_wait_dip_stays_hold():
    """
    HO-03: advisory=hold, policy=signal_long_setup, signal=wait_dip, 포지션 없음
...
@pytest.mark.asyncio
async def test_ho04_hold_policy_none_long_setup_stays_hold():
    """
    HO-04: advisory=hold, policy=none, signal=long_setup, 포지션 없음
...
async def test_ho05_hold_override_with_position_stays_hold():
    """
    HO-05: advisory=hold, policy=signal_long_setup, signal=long_setup, 포지션 있음
...
async def test_ho06_hold_override_near_expiry_suppresses_entry():
    """
    HO-06: advisory=hold, policy=signal_long_setup, signal=long_setup, 만료 0.5H 남음
...
async def test_ho08_non_hold_advisory_ignores_override_policy():
    """
    HO-08: advisory=entry_long (hold 아님), policy=signal_long_setup, signal=long_setup
    ...
    adv = _advisory(
        action="entry_long",
        confidence=0.65,
        hold_override_policy="signal_long_setup",  # 무시됨
    )
    dec, _ = _make_decision(advisory=adv)
...
async def test_ec01_entry_short_plus_long_position_direction_mismatch_logs_warning(caplog):
    ...
    with patch("core.judge.decision.rachel_advisory.datetime") as mock_dt:
        ...
        with caplog.at_level(logging.WARNING, logger="core.judge.decision.rachel_advisory"):
            ...
            result = await dec.decide(_snapshot(signal="short_setup", position=pos))
